...
<body class="bg-gray-100 dark:bg-gray-900 p-6">
    <h1 class="text-2xl font-bold mb-4 text-gray-800 dark:text-gray-200">Edit Unit Kerja: {{ $unit->unit_name }}</h1>
    ...
    <form action="{{ route('units.update', $unit->id) }}" method="POST" class="max-w-lg bg-white dark:bg-gray-800 p-6 rounded shadow dark:shadow-gray-700">
        @csrf
        @method('PUT')
        <div class="mb-4">
            <label for="unit_name" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Nama Unit</label>
            <input type="text" id="unit_name" name="unit_name" value="{{ old('unit_name', $unit->unit_name) }}" class="mt-1 block w-full border-gray-300 dark:border-gray-700 rounded-md shadow-sm focus:border-indigo-300 dark:focus:border-indigo-600 focus:ring focus:ring-indigo-200 dark:focus:ring-indigo-600 focus:ring-opacity-50 dark:bg-gray-900 dark:text-gray-200" required>
        </div>
        <div class="mb-4">
            <label for="unit_code" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Kode Unit</label>
            <input type="text" id="unit_code" name="unit_code" value="{{ old('unit_code', $unit->unit_code) }}" class="mt-1 block w-full border-gray-300 dark:border-gray-700 rounded-md shadow-sm focus:border-indigo-300 dark:focus:border-indigo-600 focus:ring focus:ring-indigo-200 dark:focus:ring-indigo-600 focus:ring-opacity-50 dark:bg-gray-900 dark:text-gray-200" required>
        </div>
        <div class="mb-4">
            <label for="unit_short" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Singkatan</label>
            <input type="text" id="unit_short" name="unit_short" value="{{ old('unit_short', $unit->unit_short) }}" class="mt-1 block w-full border-gray-300 dark:border-gray-700 rounded-md shadow-sm focus:border-indigo-300 dark:focus:border-indigo-600 focus:ring focus:ring-indigo-200 dark:focus:ring-indigo-600 focus:ring-opacity-50 dark:bg-gray-900 dark:text-gray-200" required>
        </div>
        <div class="mb-4">
            <label for="address" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Alamat</label>
            <textarea id="address" name="address" class="mt-1 block w-full border-gray-300 dark:border-gray-700 rounded-md shadow-sm focus:border-indigo-300 dark:focus:border-indigo-600 focus:ring focus:ring-indigo-200 dark:focus:ring-indigo-600 focus:ring-opacity-50 dark:bg-gray-900 dark:text-gray-200" rows="2">{{ old('address', $unit->address) }}</textarea>
        </div>
