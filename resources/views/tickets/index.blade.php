...
@section('content')
                    ...
                    <div class="block-content block-content-full overflow-x-auto">
                        <div class="row mb-4">
                            <div class="col-md-6">
                                <div class="input-group">
                                    <input type="text" name="search" id="searchInput" class="form-control"
                                        placeholder="Cari berdasarkan kode atau judul..." value="{{ request('search') }}">
                            ...
                            <div class="col-md-6 text-end">
                                <select name="per_page" id="perPageSelect" class="form-select w-auto d-inline-block"
                                    style="width: auto;">
                                    <option value="5" {{ request('per_page', 10) == 5 ? 'selected' : '' }}>5 per
                                        halaman</option>
                        ...
                        <table class="table table-bordered table-striped table-vcenter" id="ticketsTable">
                            ...
                            <tbody id="ticketsBody">
                                @forelse($tickets as $ticket)
                                    <tr class="text-center">
                                        ...
                                        <td class="p-2 text-dark">
                                            {{ $ticket->created_at->timezone('Asia/Jakarta')->format('j F Y, H:i') }}</td>
                                        ...
                                        <td class="p-2 text-dark">
                                            ...
                                            @elseif($ticket->status == 1)
                                                <span
                                                    class="fs-xs fw-semibold d-inline-block py-1 px-3 rounded-pill bg-info-light text-info">Ditugaskan</span>
                                            @else
                                                <span
                                                    class="fs-xs fw-semibold d-inline-block py-1 px-3 rounded-pill bg-success-light text-success">Selesai</span>
                                            @endif
                                        </td>
                                        <td class="p-2 text-dark">{{ $ticket->service->svc_name ?? 'Tidak ditentukan' }}
                                        </td>
                                        @if (auth()->user()->role_id == 2)
                                            <td class="p-2">
                                                @if ($ticket->status != 2)
                                                    @if ($pics->isNotEmpty())
                                                        <form action="{{ route('tickets.assign', $ticket) }}" method="POST"
                                                            ...
                                                            <select name="pic_id" class="form-select mb-2">
                                                                ...
                                                                @foreach ($pics as $pic)
                                                                    <option value="{{ $pic->id }}">
                                                                        {{ $pic->username }} ({{ $pic->pic_desc }})
                                                                    </option>
                                                                @endforeach
                                                            </select>
                                                            <button type="submit" class="btn btn-success btn-sm"
                                                                title="Tugaskan PIC"><i class="far fa-user"></i></button>
                                                        </form>
                                                        @php
                                                            ...
                                                            <ul class="list-group list-group-flush text-dark">
                                                                @foreach ($activePics as $pic)
                                                                    <li
                                                                        ...
                                                                        <p
                                                                            style="font-weight: 600; font-size: 14px; color: #333; background: #f1f3f5; padding: 4px 8px; border-radius: 4px; display: inline-block; margin: 0;">
                                                                            {{ $pic->username }}</p>
                                                                        <form
                                                                            action="{{ route('tickets.removePic', $ticket) }}"
                                                        ...
                                                        @endif
                                                    @else
                                                        <span class="text-danger">Tidak ada PIC tersedia untuk unit
                                                            ini.</span>
                                                    @endif
                                                @else
                                            ...
                                            <td class="p-2">
                                                @if ($ticket->status == 0)
                                                    <form action="{{ route('tickets.transfer', $ticket) }}" method="POST"
                                                        ...
                                                        @csrf
                                                        <select name="unit_id" id="unit_id-{{ $ticket->id }}"
                                                            class="form-select mb-2 unit-select" required>
                                                        ...
                                                        <select name="service_id" id="service_id-{{ $ticket->id }}"
                                                            class="form-select mb-2 service-select" required>
                                                            <option value="">Pilih Layanan</option>
                                                        </select>
                                                        <button type="submit"
                                                            class="btn btn-warning btn-sm">Alihkan</button>
                                                    </form>
                                                @else
                                        ...
                                        <td class="action-button p-2 text-center">
                                            <div class="mb-2">
                                                <button type="button" class="btn btn-primary btn-sm detail-btn"
                                                    ...
                                                    data-bs-target="#detailModal-{{ $ticket->id }}" title="Detail">
                                                    <i class="fas fa-eye"></i>
                                                </button>
                                            </div>
                                            <div class="mb-2">
                                                <button type="button" class="btn btn-success btn-sm chat-btn"
                                                    data-bs-toggle="modal"
                                                    data-bs-target="#chatModal-{{ $ticket->id }}" title="Pesan">
                                                    <i class="fas fa-comments"></i>
                                                </button>
                                            </div>
                                            @if (auth()->user()->role_id == 2 && $ticket->status != 2)
                                                <div>
                                                    <form id="resolve-form-{{ $ticket->id }}"
                                                        action="{{ route('tickets.resolve', $ticket) }}" method="POST"
                                                        class="d-inline">
                                                        @csrf
                                                        <button type="button"
                                                            class="btn btn-sm btn-success resolve-ticket-btn"
                                    ...
                                    @if (isset($ticket->id))
                                        <!-- Modal Detail -->
                                        <div class="modal fade bg-dark" id="detailModal-{{ $ticket->id }}"
                                            tabindex="-1" aria-labelledby="detailModalLabel-{{ $ticket->id }}"
                                            data-bs-backdrop="static" aria-hidden="true" style="font-size: 12px;">
                                            <div class="modal-dialog modal-lg">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title"
                                                            id="detailModalLabel-{{ $ticket->id }}">Detail Tiket:
                                                            {{ $ticket->ticket_code }}</h5>
                                                        <button type="button" class="btn-close"
                                                            data-bs-dismiss="modal"></button>
                                                    </div>
                                                    <div class="modal-body">
                                                        <p><strong>Waktu Dibuat:</strong>
                                                            {{ $ticket->created_at->timezone('Asia/Jakarta')->format('j F Y, H:i') }}
                                                        </p>
                                                        ...
                                                        <p><strong>Layanan:</strong>
                                                            {{ $ticket->service->svc_name ?? 'Tidak ditentukan' }}</p>
                                                        <p><strong>Deskripsi:</strong> {{ $ticket->description }}</p>
                                                        <p><strong>Unit Asal:</strong>
                                                            {{ $ticket->original_unit_id ? \App\Models\Unit::find($ticket->original_unit_id)->unit_name : $ticket->unit->unit_name ?? 'Tidak ditentukan' }}
                                                        </p>
                                                        <p><strong>Unit Saat Ini:</strong>
                                                            {{ $ticket->unit->unit_name ?? 'Tidak ditentukan' }}</p>
                                                        <div class="mt-3">
                                                            <strong>Lokasi Laporan:</strong>
                                                            @if ($ticket->latitude && $ticket->longitude)
                                                                <div class="d-flex flex-column gap-2 mt-2">
                                                                    <div class="d-flex gap-3">
                                                                        <p class="mb-0"><strong>Latitude:</strong>
                                                                            {{ $ticket->latitude }}</p>
                                                                        <p class="mb-0"><strong>Longitude:</strong>
                                                                            {{ $ticket->longitude }}</p>
                                                                    </div>
                                                                    <div id="map-{{ $ticket->id }}"
                                        ...
                                        <!-- Modal untuk Percakapan -->
                                        <div class="modal fade" id="chatModal-{{ $ticket->id }}" tabindex="-1"
                                            aria-labelledby="chatModalLabel-{{ $ticket->id }}"
                                                ...
                                                <div class="modal-content shadow-lg"
                                                    ...
                                                    <div class="modal-header d-flex justify-content-between align-items-center py-3 px-4"
                                                        ...
                                                        <h5 class="modal-title d-flex align-items-center gap-2 m-0"
                                                            id="chatModalLabel-{{ $ticket->id }}">
                                                            <i class="fas fa-ticket-alt" style="color: #2563eb;"></i>
                                                            <span
                                                                style="font-weight: 600; font-size: 1rem; color: #1f2937;">{{ $ticket->ticket_code }}</span>
                                                            @if ($ticket->status == 0)
                                                                <span class="badge bg-warning text-dark"
                                                                    style="font-size: 0.75rem; border-radius: 12px; padding: 0.25rem 0.75rem;">Pending</span>
                                                            @elseif($ticket->status == 1)
                                                                <span class="badge bg-info text-white"
                                                                    style="font-size: 0.75rem; border-radius: 12px; padding: 0.25rem 0.75rem;">Ditugaskan</span>
                                                            @else
                                                                <span class="badge bg-success text-white"
                                                                    style="font-size: 0.75rem; border-radius: 12px; padding: 0.25rem 0.75rem;">Selesai</span>
                                                            @endif
                                                        </h5>
                                                    ...
                                                    <div class="modal-body p-0"
                                                        ...
                                                        <div class="chat-container"
                                                            id="chat-container-{{ $ticket->id }}"
                                                            style="flex: 1; overflow-y: auto; padding: 1.25rem; background-color: #f9fafb;">
                                                            @forelse($ticket->responses as $response)
                                                                ...
                                                                <div class="message-wrapper mb-3"
                                                                    style="display: flex; flex-direction: column; align-items: {{ $isSender ? 'flex-end' : 'flex-start' }};">
                                                                    <div
                                                                        class="message-info d-flex align-items-center gap-2 mb-1">
                                                                        @if (!$isSender)
                                                                            <div class="avatar"
                                                                                style="width: 28px; height: 28px; border-radius: 50%; background-color: #1e3a8a; color: white; display: flex; align-items: center; justify-content: center; font-size: 0.75rem; font-weight: 600;">
                                                                                {{ substr($response->user->username, 0, 1) }}
                                                                            </div>
                                                                        @endif
                                                                        <span class="message-sender"
                                                                            style="font-weight: 600; font-size: 0.85rem; color: #374151;">
                                                                            @if ($response->user->role_id == 2)
                                                                                Sistem (Operator)
                                                                            @else
                                                                                {{ $response->user->username }}
                                                                                ({{ $response->user->role_id == 4 ? 'Pengadu' : 'PIC' }})
                                                                            @endif
                                                                        </span>
                                                                    </div>
                                                                    @if ($response->ticket_id_quote)
                                                                        <div class="message-quote"
                                                                            style="font-style: italic; {{ $isSender ? 'color: rgba(255, 255, 255, 0.8);' : 'color: #6b7280;' }} font-size: 0.8rem; margin-bottom: 6px; padding-left: 8px; border-left: 2px solid {{ $isSender ? 'rgba(255, 255, 255, 0.5)' : '#d1d5db' }};">
                                                                            "{{ $response->quotedResponse->message }}"
                                                                        </div>
                                                                    @endif
                                                                    <div class="message-box {{ $isSender ? 'sent' : 'received' }}"
                                                                        style="max-width: 80%; padding: 12px 16px; border-radius: 16px; margin-bottom: 0.5rem; box-shadow: 0 1px 2px rgba(0, 0, 0, 0.05);">
                                                                            ...
                                                                            <div class="message-attachment mb-2">
                                                                                <a href="{{ asset('storage/' . $upload->filename_path) }}"
                                                                                    ...
                                                                                    <img src="{{ asset('storage/' . $upload->filename_path) }}"
                                                                                        ...
                                                                                        style="width: 128px; height: 128px; object-fit: cover; border-radius: 8px; {{ !$isSender ? 'border: 1px solid #e5e7eb;' : '' }}">
                                                                                </a>
                                                                            </div>
                                                                        @empty
                                                                        @endforelse
                                                                        <p class="mb-0"
                                                                            style="line-height: 1.5; font-size: 0.9rem;">
                                                                            {{ $response->message }}</p>
                                                                        <span class="message-time"
                                                                            style="font-size: 0.7rem; {{ $isSender ? 'color: rgba(255, 255, 255, 0.85);' : 'color: #6b7280;' }} display: block; text-align: right; margin-top: 4px;">
                                                                            {{ $response->created_at->timezone('Asia/Jakarta')->format('d M Y, H:i') }}
                                                                        </span>
                                                                    </div>
                                                                </div>
                                                            @empty
                                                                <p class="text-muted text-center">Belum ada percakapan
                                                                    untuk tiket ini.</p>
                                                            @endforelse
                                                        </div>

                                                        @php
                                                            $hasPegawaiResponse = $ticket
                                                                ->responses()
                                                            ...
                                                            $isPicAssignedToTicket =
                                                                auth()->user()->role_id == 3
                                                                    ? DB::table('ticket_pic')
                                                                        ...
                                                                        ->where('ticket_pic.ticket_id', $ticket->id)
                                                                        ->where('pics.user_id', auth()->user()->id)
                                                                        ->where('ticket_pic.pic_stats', 'active')
                                                                        ->exists()
                                                                    : false;

                                                            $pengaduMessagesSinceLastPegawai = $ticket
                                                                ->responses()
                                                                ->where('user_id', auth()->user()->id)
                                                                ->count();
                                                            if ($hasPegawaiResponse) {
                                                                $lastPegawaiResponse = $ticket
                                                                    ->responses()
                                                                    ->where('user_id', '!=', auth()->user()->id)
                                                                    ->whereHas('user', function ($query) {
                                                                        $query->where('role_id', 3);
                                                                ...
                                                                $pengaduMessagesSinceLastPegawai = $ticket
                                                                    ...
                                                                    ->where(
                                                                        'created_at',
                                                                        '>',
                                                                        $lastPegawaiResponse->created_at,
                                                                    )
                                                                    ->count();
                                                            }
                                                        ...
                                                        @if (
                                                            $ticket->status != 2 &&
                                                                (auth()->user()->role_id == 2 ||
                                                                    (auth()->user()->role_id == 3 && $isPicAssignedToTicket) ||
                                                                    (auth()->user()->role_id == 4 &&
                                                                        $ticket->user_id == auth()->user()->id &&
                                                                        ($hasPicAssigned || $hasOperatorResponse) &&
                                                                        $pengaduMessagesSinceLastPegawai < $messageLimit)))
                                                            <div class="reply-container"
                                                                style="background-color: white; border-top: 1px solid #e5e7eb; padding: 1rem;">
                                                                <form id="reply-form-{{ $ticket->id }}"
                                                                    action="{{ route('tickets.respond', $ticket) }}"
                                                                    method="POST" enctype="multipart/form-data"
                                                                    class="reply-form">
                                                                    @csrf
                                                                    <div class="reply-input-row d-flex gap-2 mb-2">
                                                                        <textarea class="form-control" name="message" placeholder="Ketik pesan Anda di sini..." required
                                                                            style="border-radius: 24px; border-color: #d1d5db; padding: 12px 16px; font-size: 0.9rem; resize: none;"></textarea>
                                                                        <button type="submit"
                                                                    ...
                                                                    <div
                                                                        class="attachment-row d-flex align-items-center gap-2">
                                                                        <button class="btn btn-outline-primary"
                                                                            type="button"
                                                                            id="custom-button-{{ $ticket->id }}"
                                                                            style="border-radius: 20px; padding: 6px 14px; font-size: 0.85rem; display: flex; align-items: center; gap: 6px;">
                                                                            <i class="fas fa-paperclip"></i>
                                                                            <span>Lampirkan File</span>
                                                                        </button>
                                                                        <span id="file-name-{{ $ticket->id }}"
                                                                            class="text-muted"
                                                                            style="font-size: 0.85rem; flex: 1; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">Tidak
                                                                            ada file dipilih</span>
                                                                        <input type="file" name="images[]"
                                                                            id="images-{{ $ticket->id }}" multiple
                                                                            class="form-control d-none">
                                                                    </div>
                                                                </form>
                                                            </div>
                                                        @elseif ($ticket->status == 2)
                                                            <div class="reply-container text-center"
                                                                style="background-color: white; border-top: 1px solid #e5e7eb; padding: 1rem;">
                                                                <p class="text-muted mb-0" style="font-size: 0.9rem;">
                                                                    Layanan telah selesai. Tidak dapat mengirim pesan lagi.
                                                                </p>
                                                            </div>
                                                        @endif
                                                    </div>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="{{ auth()->user()->role_id == 2 ? 10 : 8 }}"
                                            class="p-2 text-dark text-center">
    ...
    <script>
        function attachResolveButtonListeners() {
        ...
        document.addEventListener('DOMContentLoaded', function() {
                ...
                fetch(`/tickets?page=${page}&search=${encodeURIComponent(search)}&status_filter=${statusFilter}&per_page=${perPage}`, {
                    ...
                    .then(data => {
                        ...
                        if (data.tickets.data.length === 0) {
                            ...
                            row.innerHTML = `
                        <td colspan="${@json(auth()->user()->role_id == 2 ? 10 : 8)}" class="p-2 text-dark text-center">
                            Tidak ada laporan yang ditemukan.
                        </td>
                    `;
                            ...
                            <td class="action-button p-2 text-center">
                                ...
                                @if (auth()->user()->role_id == 2)
                                    ${ticket.status != 2 ? `
                                                <div>
                                                    <form id="resolve-form-${ticket.id}" action="/tickets/resolve/${ticket.id}" method="POST" class="d-inline">
                                                        <input type="hidden" name="_token" value="${document.querySelector('meta[name="csrf-token"]').content}">
                                                        <button type="button" class="btn btn-sm btn-success resolve-ticket-btn" data-ticket-id="${ticket.id}" title="Selesaikan Tiket">
                                                            <i class="fas fa-check"></i>
                                                        </button>
                                                    </form>
                                                </div>
                        ...
                        attachResolveButtonListeners(); // Attach listeners for dynamic resolve buttons

                        // Reattach modal event listeners
                        document.querySelectorAll('.detail-btn').forEach(btn => {
                            btn.addEventListener('click', () => {
                                console.log('Detail button clicked for ticket:', btn
                        ...
                        document.querySelectorAll('.chat-btn').forEach(btn => {
                            btn.addEventListener('click', () => {
                                console.log('Chat button clicked for ticket:', btn.getAttribute(
                                    'data-bs-target'));
                            });
                        });
                    })
                    .catch(error => {
                        ...
                        ticketsBody.innerHTML =
                            `<tr><td colspan="${@json(auth()->user()->role_id == 2 ? 10 : 8)}" class="p-2 text-dark text-center">Terjadi kesalahan saat memuat data tiket. Silakan coba lagi.</td></tr>`;
                ...
                paginationLinks.querySelectorAll('a').forEach(link => {
                    link.addEventListener('click', function(e) {
                        ...
                        if (!this.classList.contains('disabled')) {
                            const page = this.getAttribute('data-page') || new URL(this.href)
                    ...
                    fetch(this.action, {
                        ...
                        .then(data => {
                            if (data.success) {
                                ...
                                messageBox.className = 'message-box sent';
                                messageBox.style.maxWidth = '80%';
                                messageBox.style.padding = '12px 16px';
                                messageBox.style.borderRadius = '16px';
                                messageBox.style.marginBottom = '0.5rem';
                                messageBox.style.boxShadow = '0 1px 2px rgba(0, 0, 0, 0.05)';
                                ...
                                if (data.uploads && data.uploads.length > 0) {
                                    ...
                                    <div class="message-attachment mb-2">
                                        <a href="/storage/${upload.filename_path}" target="_blank">
                                            <img src="/storage/${upload.filename_path}" alt="${upload.filename_ori}" style="width: 128px; height: 128px; object-fit: cover; border-radius: 8px;">
                                        </a>
                                    </div>
                                `;
                                    });
                                ...
                                messageContent += `
                            <p class="mb-0" style="line-height: 1.5; font-size: 0.9rem;">${formData.get('message')}</p>
                            <span class="message-time" style="font-size: 0.7rem; color: rgba(255, 255, 255, 0.85); display: block; text-align: right; margin-top: 4px;">
                                ${new Date().toLocaleString('id-ID', { timeZone: 'Asia/Jakarta', day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
                            </span>
                        `;
            ...
            attachPaginationListeners();
            attachTransferFormListeners();
            attachResolveButtonListeners(); // Attach listeners for static resolve buttons
        });
    </script>
@endsection
