...
<head>
    ...
    <title>Tiket Ditugaskan - Helpdesk Kota</title>
    <link href="https://cdn.jsdelivr.net/npm/tailwindcss@3.4/dist/tailwind.min.css" rel="stylesheet">
    @vite(['resources/css/app.css', 'resources/js/app.js'])
</head>
<body class="bg-gray-100 dark:bg-gray-900 p-6">
    <h1 class="text-2xl font-bold mb-4 text-gray-800 dark:text-gray-200">Tiket Ditugaskan</h1>
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full bg-white dark:bg-gray-800 border dark:border-gray-700">
            ...
            <tbody>
                @forelse($tickets as $ticket)
                    <tr>
                        ...
                        <td class="py-2 px-4 border-b dark:border-gray-600">
                            <!-- Form untuk memberikan respons -->
                            <form action="{{ route('tickets.respond', $ticket) }}" method="POST" enctype="multipart/form-data" class="mb-2">
                                @csrf
                                <textarea name="message" class="border p-2 rounded dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200 w-full mb-2" placeholder="Masukkan respons..." required></textarea>
                                ...
                                <button type="submit" class="bg-blue-500 text-white px-2 py-1 rounded hover:bg-blue-600 dark:bg-blue-600 dark:hover:bg-blue-700">Kirim Respons</button>
                            </form>
                            ...
                            <form action="{{ route('tickets.update', $ticket) }}" method="POST">
                                ...
                                <button type="submit" class="bg-green-500 text-white px-2 py-1 rounded hover:bg-green-600 dark:bg-green-600 dark:hover:bg-green-700">Update Status</button>
                            </form>
                        </td>
                    </tr>
                    <!-- Tampilkan lampiran awal dari aduan -->
                    @if ($ticket->uploads->isNotEmpty())
                        <tr>
                            <td colspan="5" class="py-2 px-4 border-b dark:border-gray-600">
                                <div class="ml-4">
                                    <h4 class="text-md font-semibold text-gray-800 dark:text-gray-200">Lampiran Awal Aduan:</h4>
                                    @forelse($ticket->uploads as $upload)
                                        <div class="mt-2">
                                    ...
                                    @empty
                                        <p class="text-sm text-gray-500 dark:text-gray-400">Tidak ada lampiran awal.</p>
                                    @endforelse
                                </div>
                            </td>
                        </tr>
                    ...
                    <tr>
                        <td colspan="5" class="py-2 px-4 border-b dark:border-gray-600">
                            <div class="ml-4">
                                <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-200">Riwayat Percakapan untuk {{ $ticket->ticket_code }}</h3>
                                @forelse($ticket->responses as $response)
                                    <div class="border-l-4 pl-4 mt-2 {{ $response->user->role_id == 4 ? 'border-green-500' : 'border-blue-500' }}">
                                        ...
                                        <!-- Tampilkan gambar yang dilampirkan di respons -->
                                        ...
                                        @endforelse
                                        <!-- Form untuk PIC memberikan balasan -->
                                        @if (auth()->user()->role_id == 3 && $ticket->status != 2)
                                            <form action="{{ route('tickets.respond', $ticket) }}" method="POST" enctype="multipart/form-data" class="mt-2">
                                                @csrf
                                                <textarea name="message" class="border p-2 rounded dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200 w-full mb-2" placeholder="Masukkan balasan..." required></textarea>
                                                <input type="file" name="images[]" multiple class="border p-1 rounded dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200 mb-2">
                                                <button type="submit" class="bg-blue-500 text-white px-2 py-1 rounded hover:bg-blue-600 dark:bg-blue-600 dark:hover:bg-blue-700">Kirim Balasan</button>
                                            </form>
                                        @endif
                                    </div>
                                @empty
                ...
                @empty
                    <tr>
                        <td colspan="5" class="py-2 px-4 border-b dark:border-gray-600 text-gray-800 dark:text-gray-200 text-center">Tidak ada tiket yang ditugaskan.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
